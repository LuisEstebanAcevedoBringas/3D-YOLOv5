...
class LoadImagesAndLabels(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if mosaic: # Load mosaic
            ...
            path_imgs_mosaic = []
            indexu = [index] + random.choices(self.indices, k=3)

            for k in range(len(indexu)):
                path_imgs_mosaic.append(self.im_files[indexu[k]])

            final_stacks = generate_n_stacks(path_imgs_mosaic, n)

            for i in range(n): #n = al tamaño del volumen
                img, labels = self.load_mosaic_Luiggi(eme, s_rpv, yc, xc, indexu, final_stacks[i])
            ...
            for i in paths_stack:
                # Load image
                img, (h0, w0), (h, w) = self.load_image_Luiggi(i)
                ...
                if self.augment:
                    img, labels = random_perspective_Luiggi(img,
                                                    eme,
                                                    s_rpv,
                                                    labels,
                                                    perspective=hyp['perspective'],
        ...
        if self.augment:
            ...
            final_volume = final_volume.permute(3, 0, 1, 2)
...
# ------------------------ Luiggi ------------------------
    ...
    def load_mosaic_Luiggi(self ,eme, s_rpv, yc, xc, indices, final_stacks):
        ...
        for i, index in enumerate(indices):
            # Load image
            video_n, video_i, fp = get_info(final_stacks[i])
            img, _, (h, w) = self.load_image_Luiggi(fp + "{}/".format(video_n) + "{}_{:06d}.jpg".format(video_n, video_i))
        ...
        # Augment
        #Nunca entra al copypaste por la probabilidad del hyp
        img4, labels4, segments4 = copy_paste(img4, labels4, segments4, p=self.hyp['copy_paste'])
        img4, labels4, true_imgs = random_perspective_Luiggi(img4,
                                            eme,
                                            s_rpv,
                                            labels4,
                                            segments4,
...
def draw_bboxes(annots, img):
    draw = ImageDraw.Draw(img)
    for annot in annots:
        _, class_id, x_center, y_center, width, height = annot
...
# def save_json(data):
#     with open("dataset_IPN_Hand_new.json", 'w') as f:
#         json.dump(data, f)
...
def generate_stack(path, clip_size):
    ...
    if video_i < clip_size:
        dif = clip_size - video_i
        for i in range(video_i, 0, -1):
            data.append(img_paths + "{}/".format(video_n) + "{}_{:06d}.jpg".format(video_n, i))
        for _ in range(dif):
            data.append(img_paths + "{}/".format(video_n) + "{}_{:06d}.jpg".format(video_n, 1))

    else:
        for _ in range(video_i, (video_i - (clip_size)), -1):
            data.append(img_paths + "{}/".format(video_n) + "{}_{:06d}.jpg".format(video_n, video_i))
            video_i -= 1

    # save_json(data)        
    return data

def generate_n_stacks(paths, clip_size): # We return all the volumes of images for the stacks
    final_mosaic_stacks = []
    data = []
    # img_paths = "/host/space0/gibran/dataset/HandGestures/IPN_new_dataset/frames/"
    img_paths = "F:/IPN_Hand/frames/"
    ...
    for n in paths:  
        stack_n = []
        video_n, video_i, _ = get_info(n)
        if video_i < clip_size:
            dif = clip_size - video_i
            for i in range(video_i, 0, -1):
                stack_n.append(img_paths + "{}/".format(video_n) + "{}_{:06d}.jpg".format(video_n, i))
            for _ in range(dif):
                stack_n.append(img_paths + "{}/".format(video_n) + "{}_{:06d}.jpg".format(video_n, 1))

        else:
            for _ in range(video_i, (video_i - (clip_size)), -1):
                stack_n.append(img_paths + "{}/".format(video_n) + "{}_{:06d}.jpg".format(video_n, video_i))
                video_i -= 1
        data.append(stack_n)

    for row in range(clip_size):
        temp = []
        for column in range(4):
            temp.append(data[column][row])
        final_mosaic_stacks.append(temp)

    # save_json(final_mosaic_stacks)   
    return final_mosaic_stacks
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-path', type=str, default='C:/Users/Luis Bringas/Desktop/gracias_por_el_cooler/dummy_set/', help='dataset path')
    ...
    idx_ = 58
    data_i = dataset.__getitem__(idx_)
    print(f'\nlabels [{idx_+1}]:', data_i[1])
    print(f'path [{idx_+1}]:', data_i[2])
    generate_stack(data_i[2], 5)
    #save_image_with_bboxes(data_i[1].tolist(), data_i[0], f'image_{idx_+1}.jpg')
