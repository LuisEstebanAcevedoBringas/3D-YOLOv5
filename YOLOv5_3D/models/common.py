...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        # print("input bottleneck: ", x.size())
        # print("output bottleneck: ", (x + self.cv2(self.cv1(x)) if self.add else self.cv2(self.cv1(x))).size())
        # pdb.set_trace()
        return x + self.cv2(self.cv1(x)) if self.add else self.cv2(self.cv1(x))
...
class C3(nn.Module):
    ...
    def forward(self, x):
        # print("input C3:" , x.size())
        # print("output C3:", self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1)).size())
        # pdb.set_trace()
...
convnext_other_depths = [3, 3, 27, 3]
...
class StemConv(nn.Module):
    # Stem convolution from ConvNeXt 
    def __init__(self, c1, c2, k=4, s=4):
        super().__init__()
        self.conv = nn.Conv2d(c1, c2, kernel_size=k, stride=s)
        self.norm = LayerNorm(c2, eps=1e-6, data_format="channels_first")

    def forward(self, x):
        # print ("stem2D: ",self.norm(self.conv(x)).size())
        return self.norm(self.conv(x))


class DownSample(nn.Module):
    # Downsample layer from ConvNeXt
    ...
    def forward(self, x):
        # print ("DownSample2D: ",self.conv(self.norm(x)).size())
        return self.conv(self.norm(x))


class ConvNeXtStage(nn.Module):
    # ConvNeXt stage block 
    def __init__(self, c1, idx, drop_path_rate=0.1, layer_scale_init_value=1e-6):
        super().__init__()
        depths = convnext_tiny_depths if drop_path_rate == 0.1 else convnext_other_depths

        dp_rates = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]
        layers = []
        cur = 0 if idx < 1 else sum(depths[:idx]) #para el primer depths + depths[i]
        for i in range(depths[idx]):
            layers.append(Block(dim=c1, drop_path=dp_rates[cur + i], layer_scale_init_value=layer_scale_init_value))
        self.stage = nn.Sequential(*layers)

    def forward(self, x):
        # print ("stage2D: ",self.stage(x).size())
        return self.stage(x)
